...
**Failure-path fixtures.** Three tests in the group cover the ways a session must *fail*: expiry, a token presented to the wrong worker, and a token replayed by a different principal. Each needs a worker the default fixture can't stand in for, so each is gated on an optional runner fixture — `conformance_http_sticky_short_ttl_port`, `conformance_http_sticky_peer_ports`, and `conformance_http_sticky_auth_port` respectively. See [`docs/sticky-sessions-spec.md`](sticky-sessions-spec.md) §9.1 for the exact shapes and [`tests/conftest.py`](https://github.com/Query-farm/vgi-rpc-python/blob/main/tests/conftest.py) for the Python reference (each is a few lines on top of the existing worker, driven by the `--sticky-ttl` / `--token-key` / `--sticky-auth` flags in `tests/serve_conformance_http.py`). Ports that omit them stay conformant on the rest of the group but leave their session-loss paths unverified.
